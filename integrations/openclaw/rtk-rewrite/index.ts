/**
 * Butler-vendored RTK Rewrite Plugin for OpenClaw.
 *
 * Origin: rtk-ai/rtk openclaw plugin, vendored so Butler can install it
...
export default function register(api: any) {
  ...
  if (!checkRtk()) {
    console.warn("[aibutler-rtk] rtk binary not found in PATH — plugin disabled");
    return;
  }
  ...
  api.on(
    ...
    (event: { toolName: string; params: Record<string, unknown> }) => {
      ...
      if (verbose) {
        console.log(`[aibutler-rtk] ${command} -> ${rewritten}`);
      }
